...
def cmsDriver_command(filein = None, campaign = 'Run3Summer22', number = 1000) :
    ...
    if file_base.endswith('DRPremix.root'):
        file_base = file_base[:-len('DRPremix.root')] 
    out_config = f'{file_base}AOD_cfg.py'
    out_root = f'{file_base}AOD.root'   
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--filein", 
                           default=None, 
                           help="Input file from DRPremix stage to use as input for the AOD step.")
